#!/usr/bin/env bash
trap "echo Likely problem is that ASG must terminate completely before killing VPC. try just the killVPC.sh again once the instances shutdown and the ASGs go away.  Or delete using console; exit" ERR
