...
aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block $ROXY_BASE_SUBNET3_CIDR --availability-zone $ROXY_BASE_REGION'c' > lastSubNet3.res
...
aws ec2 authorize-security-group-ingress --group-id $DBSG_ID --port $ROXY_BASE_RDS_PORT --protocol tcp --source-group $EC2SG_ID > lastNewRDSIngress.res
